...
//        fos.close();
		...
		*/
        ...
        EasemobRestAPIFactory factory = ClientContext.getInstance().init(ClientContext.INIT_FROM_PROPERTIES).getAPIFactory();
        IMUserAPI user = (IMUserAPI) factory.newInstance(EasemobRestAPIFactory.USER_CLASS);
//        BodyWrapper userBody = new IMUserBody("d_15757118231", "123456", "d_15757118231");
//        user.createNewIMUserSingle(userBody);

//        Object object = user.deleteIMUserByUserName("d_15757118231");
        ResponseWrapper responseWrapper = (ResponseWrapper)user.deleteIMUserByUserName("d_15757118231");

        Integer responseStatus = responseWrapper.getResponseStatus();
        if (responseStatus == 200) {
//            apiResult.success(responseWrapper);
